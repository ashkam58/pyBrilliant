...
- The outer loop doubles `size` each time: sizes 1,2,4,... up to ~n → about log n iterations.
- For a given size, the inner loop steps by `size` and runs about n/size iterations, each O(1).
- Work per outer iteration: n/size * O(1) → O(n/size).
- Sum over sizes: sum_{k=0..log n} O(n / 2^k) = O(n) * sum_{k} 1/2^k = O(n).
